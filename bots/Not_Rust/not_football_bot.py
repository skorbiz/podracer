...
def goalKeeping(objectives, pod_states, left_side, race_pod):
    x = 0
    y = 0
    thrust = 75
    ...
    goal_size = 1600
    goal_center_x =  0 if left_side else 16000
    goal_center_y =  4500

    football_x, football_y = objectives

    diff_x = football_x- goal_center_x 
    diff_y = football_y- goal_center_y 

    football_dist = dist(diff_x,diff_y)
    football_player_dist = dist(x1-football_x,y1-football_y)

    angle_from_goal = atan2(diff_y,diff_x)

    min_defend_dist =1600
    max_defend_dist = 2000

    max_ball_dist = 8000
    min_ball_dist = 1800

    if(football_dist >max_ball_dist ):
        football_dist = max_ball_dist
    if( football_dist< min_ball_dist):
       football_dist =min_ball_dist
    # np.clip(football_dist,min_ball_dist,max_ball_dist)

# if not race_pod.isBoostAvailable() and race_pod.isShieldAvailable() and vel > 200 and i > 60 and  dist_to_enemy_1 < 1500:
    #             thrust = "SHIELD"
    #             race_pod.triggerShield()

    if (football_dist < 4000 and football_player_dist < 2000 and race_pod.isShieldAvailable()):
        thrust = "SHIELD"
        race_pod.triggerShield()
    elif(football_dist < 2500):
        thrust = 100
   

    dist_calculation = (max_defend_dist-min_defend_dist)*((football_dist-max_ball_dist)/(min_ball_dist-max_ball_dist))+ min_defend_dist

    debug("Dist_calc: {}".format(dist_calculation))

    target_x = goal_center_x + np.cos(angle_from_goal)*dist_calculation
    target_y = goal_center_y + np.sin(angle_from_goal)*dist_calculation 
...
while True:
    ...
    if i<3:
        left_side = pod_states[0] < 8000
    debug("is_left: {}".format(left_side))
    ...
    if gamemode == 'FOOTBALL':
        ...
        # target_y = football_y
        try:
            target_x, target_y, thrust = goalKeeping(objectives, pod_states, left_side, race_pod)
        except:
            traceback.print_exc()
