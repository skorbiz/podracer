...
checkpoints = []
attack_area_top = True # change for the other player
...
while True:
    ...
    if is_ball_in_attacking_area(football_x, football_y, opponent_goal_right, attack_area_top):
        print("BAll in attakcing. Top stricker {}".format(attack_area_top), file=sys.stderr, flush=True)

        target_x = objectives[0]
        target_y = objectives[1]
        thrust = 100

    else:
        print("BAll not in attakcing. Top stricker {}".format(attack_area_top), file=sys.stderr, flush=True)
    ...
    if len(pod_states) == 6:
        print(str(target_x_top) + " " + str(target_y_top) + " " + str(thrust_top) + " " + str(target_x) + " " + str(target_y) + " " + str(thrust), flush=True)
